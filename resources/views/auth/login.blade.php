    ...
    <section class="order-1 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-8" aria-labelledby="login-title">
      ...
      <form class="mt-7 grid gap-5" action="{{ route('login.store') }}" method="post">
        ...
        <div>
          ...
          <input id="login-identity" name="identity" type="text" value="{{ old('identity') }}" required autofocus autocomplete="username" class="mt-2 h-14 w-full rounded-2xl border bg-slate-50 px-4 text-base font-bold text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-[#2563EB] focus:bg-white focus:ring-4 focus:ring-blue-100 {{ $errors->has('identity') ? 'border-red-300' : 'border-slate-200' }}" />
        </div>
        <div>
      ...
      <div class="mt-4 grid gap-3 sm:grid-cols-2" aria-label="روابط دخول سريعة">
        <a href="{{ route('dashboard') }}" class="inline-flex h-12 items-center justify-center rounded-2xl border border-blue-100 bg-blue-50 px-4 text-sm font-extrabold text-[#2563EB] hover:bg-blue-100 focus:outline-none focus:ring-4 focus:ring-blue-100">دخول صاحب نشاط</a>
        <a href="{{ route('dashboard') }}" class="inline-flex h-12 items-center justify-center rounded-2xl border border-slate-200 bg-white px-4 text-sm font-extrabold text-slate-800 hover:bg-slate-50 focus:outline-none focus:ring-4 focus:ring-blue-100">دخول الإدارة</a>
      </div>
